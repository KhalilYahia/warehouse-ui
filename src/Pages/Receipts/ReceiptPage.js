...
function ReceiptPage() {
  ...
  const handleSubmit = async (e) => {
      setLoading(true);
      try {
      const res = await axios.post(ApiURL+'Receipt/Search', formData, {
        headers: {
          'Accept': 'text/plain',
  ...
  return (
    ...
    <legend>фильтр</legend>
     <div className='filter-kh'>
      ...
      <Select
        ...
        isMulti
        value={DocumentsNumberoptions.filter(opt => formData.inboundDocumentIds.includes(opt.value))}
    ...
    handleChange(
      ...
      <button onClick={() => {handleSubmit();}}>Применить</button>
       ...
       <button className="btn btn-add" onClick={() => {navigate('/Receipts/addReceipt')}}>+ Добавить</button>
        
      <ReceiptTable data={results}/>
      <ToastContainer />
