...
const EditUnitPage= () =>  {
 ...
 useEffect(() => {
    ...
    const fetchData = async () => {       
        axios.get(ApiURL +'Unit/GetById?id='+id).then((data)=>{    
  ...
  const showError = (msg) => {
   ...
   const handleSubmit = async (dataToSend = formData) => {
    ...
    if(!(formData.name.length<1||formData.name.length>50 ))
    {
      setLoading(true);
      try {
      const res = await axios.put(ApiURL+'Unit/Update', dataToSend, {
        headers: {
          'Accept': 'text/plain',
  ...
  if (error.response && error.response.data) {
   ...
   const handleDelete = async (e) => {
      setLoading(true);
      try {
      const res = await axios.delete(ApiURL+'Unit/Delete?id='+formData.id,  {
        headers: {
          'Accept': 'text/plain',
          'Content-Type': 'application/json'
        }
